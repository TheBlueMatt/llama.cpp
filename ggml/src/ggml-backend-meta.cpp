...
static void ggml_backend_meta_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    switch (split_state.axis) {
        ...
        case GGML_BACKEND_SPLIT_AXIS_1:
        case GGML_BACKEND_SPLIT_AXIS_2: {
            // The full tensor is a stack of outer slices of size chunk_size_full = nb[split+1].
            // Within one outer slice, bytes are laid out as: for each segment s, for each device
            // j, the portion corresponding to split_state.ne[s*n_bufs + j] elements along the
            // split axis. On device j, the simple tensor stores its portion of those slices
            // contiguously with outer stride nb[split+1]_simple; within one outer slice, device
            // j's segments are packed in order. Splice the (possibly partial) outer-slice range
            // from data into the simple tensors. Handles n_segments == 1 as a special case where
            // each device contributes one span per outer slice.
            const size_t chunk_size_full = tensor->nb[split_state.axis + 1];
            GGML_ASSERT(offset % chunk_size_full == 0);
            GGML_ASSERT(size   % chunk_size_full == 0);
            const int64_t i_start =  offset        /chunk_size_full;
            const int64_t i_stop  = (offset + size)/chunk_size_full;
            // dev_col_offset[j]: bytes consumed on device j within one outer slice by earlier
            // segments. src_col_offset: bytes already read from each source outer slice by
            // earlier (s, j) pairs.
            std::vector<size_t> dev_col_offset(n_bufs, 0);
            size_t src_col_offset = 0;
            for (size_t s = 0; s < split_state.n_segments; s++) {
                for (size_t j = 0; j < n_bufs; j++) {
                    const int64_t ne_sj = split_state.ne[s*n_bufs + j];
                    if (ne_sj == 0) continue;
                    ggml_tensor * simple_tensor = ggml_backend_meta_buffer_simple_tensor(tensor, j);
                    const size_t chunk_size_j = simple_tensor->nb[split_state.axis + 1];
                    const size_t width_sj = split_state.axis == GGML_BACKEND_SPLIT_AXIS_0
                        ? ggml_row_size(tensor->type, ne_sj)
                        : ne_sj * tensor->nb[split_state.axis];
                    const size_t simple_offset = i_start * chunk_size_j + dev_col_offset[j];
                    ggml_backend_tensor_set_2d(simple_tensor, (const char *) data + src_col_offset,
                        simple_offset, width_sj, i_stop - i_start, chunk_size_j, chunk_size_full);
                    src_col_offset    += width_sj;
                    dev_col_offset[j] += width_sj;
                }
            }
            GGML_ASSERT(src_col_offset == chunk_size_full);
        } break;
        case GGML_BACKEND_SPLIT_AXIS_MIRRORED: {
            for (size_t j = 0; j < n_bufs; j++) {
                ggml_tensor * simple_tensor = ggml_backend_meta_buffer_simple_tensor(tensor, j);
...
static void ggml_backend_meta_buffer_get_tensor(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    switch (split_state.axis) {
        ...
        case GGML_BACKEND_SPLIT_AXIS_1:
        case GGML_BACKEND_SPLIT_AXIS_2: {
            // Inverse of ggml_backend_meta_buffer_set_tensor — see the comment there for the
            // full-tensor / simple-tensor layout. Splice a (possibly partial) outer-slice range
            // out of the simple tensors and into data. Handles n_segments == 1 as a special case
            // where each device contributes one span per outer slice.
            const size_t chunk_size_full = tensor->nb[split_state.axis + 1];
            GGML_ASSERT(offset % chunk_size_full == 0);
            GGML_ASSERT(size   % chunk_size_full == 0);
            const int64_t i_start =  offset        /chunk_size_full;
            ...
            for (size_t s = 0; s < split_state.n_segments; s++) {
                for (size_t j = 0; j < n_bufs; j++) {
                    ...
                    const size_t width_sj = split_state.axis == GGML_BACKEND_SPLIT_AXIS_0
                        ...
                        : ne_sj * tensor->nb[split_state.axis];
                    const size_t simple_offset = i_start * chunk_size_j + dev_col_offset[j];
                    ggml_backend_tensor_get_2d(simple_tensor, (char *) data + dst_col_offset,
                        simple_offset, width_sj, i_stop - i_start, chunk_size_j, chunk_size_full);
                    dst_col_offset    += width_sj;
                    dev_col_offset[j] += width_sj;
                }
            }
            GGML_ASSERT(dst_col_offset == chunk_size_full);
        } break;
        case GGML_BACKEND_SPLIT_AXIS_MIRRORED: {
