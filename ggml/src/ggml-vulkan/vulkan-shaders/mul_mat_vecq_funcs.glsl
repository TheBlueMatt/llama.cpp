...
FLOAT_TYPE mmvq_dot_product(const uint ib_a, const uint iqs) {
    ...
    const i32vec4 qs_a0 = repack4(ib_a, iqs * 4);
    i32vec4 qs_a1;
    if (K_PER_ITER == 32) {
	qs_a1 = repack4(ib_a, (iqs + 1) * 4);
    }

    const uint8_t scale0 = get_scale(ib_a, iqs * 4);
    uint8_t scale1;
    if (K_PER_ITER == 32) {
        scale1 = get_scale(ib_a, (iqs + 1) * 4);
    }
...
FLOAT_TYPE mmvq_dot_product(const uint ib_a, const uint iqs) {
    ...
    const float d_scale0 = get_d_scale(ib_a, iqs * 4);
    float d_scale1;
    if (K_PER_ITER == 32) {
        d_scale1 = get_d_scale(ib_a, (iqs + 1) * 4);
    }
    ...
    float scale_sum = d_scale0 * float(q_sum0);
    ...
    if (K_PER_ITER == 32) {
        ...
        scale_sum += d_scale1 * float(q_sum1);
    }

    return FLOAT_TYPE(float(cache_b_ds.x) * scale_sum);
}
#endif
...
FLOAT_TYPE mmvq_dot_product(const uint ib_a, const uint iqs) {
    ...
    float scale_ds_y;
    if (K_PER_ITER == 16) {
        scale_ds_y = float(dm_scale.y) * float(cache_b_ds.y / 2);
    } else {
        scale_ds_y = float(dm_scale.y) * float(cache_b_ds.y);
    }

    return FLOAT_TYPE(float(cache_b_ds.x) * float(dm_scale.x) * float(q_sum) - scale_ds_y);
}
#endif
...
FLOAT_TYPE mmvq_dot_product(const uint ib_a, const uint iqs) {
    ...
    q_sum0 += dotPacked4x8EXT(qs_a0.z, cache_b_qs[2]);
    q_sum0 += dotPacked4x8EXT(qs_a0.w, cache_b_qs[3]);
    float scaled_sum = float(d_scale0) * float(q_sum0);
    ...
    if (K_PER_ITER == 32) {
        ...
        q_sum1 += dotPacked4x8EXT(qs_a1.z, cache_b_qs[6]);
        q_sum1 += dotPacked4x8EXT(qs_a1.w, cache_b_qs[7]);
        scaled_sum += float(d_scale1) * float(q_sum1);
    }

    return FLOAT_TYPE(float(cache_b_ds.x) * scaled_sum);
}
#endif
