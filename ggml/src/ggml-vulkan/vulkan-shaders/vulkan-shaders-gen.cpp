...
void process_shaders() {
    ...
    for (const auto& tname : type_names) {
        ...
        if (is_legacy_quant(tname) || tname == "mxfp4" || is_k_quant(tname) || tname == "iq1_s" || tname == "iq1_m") {
            ...
            // Shared memory staging variants (all legacy quants + mxfp4)
            if (is_legacy_quant(tname) || tname == "mxfp4") {
                string_to_spv("mul_mat_vec_" + tname + "_q8_1_f32_shmem_staging", "mul_mat_vecq.comp", merge_maps(base_dict, {{data_a_key, "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}, {"FLOAT_TYPE_VEC2", "vec2"}, {"ACC_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}, {"USE_SHMEM_STAGING", "1"}}));
                string_to_spv("mul_mat_vec_id_" + tname + "_q8_1_f32_shmem_staging", "mul_mat_vecq.comp", merge_maps(base_dict, {{"MUL_MAT_ID", "1"}, {data_a_key, "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}, {"FLOAT_TYPE_VEC2", "vec2"}, {"ACC_TYPE", "float"}, {"USE_SUBGROUP_ADD", "1"}, {"USE_SHMEM_STAGING", "1"}}));
            }
        }
#endif
